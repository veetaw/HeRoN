...
def calculate_reward(ideal_instructions, suggested_action):
    ...
    # Prova a parsare suggested_action come JSON
    suggested_att_raw = ""
    suggested_sup_raw = ""
    
    try:
        # Primo tentativo: JSON normale
    ...
    if not suggested_att_action and not suggested_sup_action:
        return -3.0
    
    # Normalizza le azioni
...
ref_model = copy.deepcopy(model).to(device)  # Reference model per KL divergence
...
class ValueHead(torch.nn.Module):
    """Value head aggiunto al decoder per stimare i rewards."""
...
class SimplePPOTrainer:
    def __init__(self, model, ref_model, tokenizer, config, device):
        ...
        # Value head per il modello principale
    ...
    def compute_kl_penalty(self, query_ids, response_ids):
        """Calcola la penalità KL tra il modello e il reference model."""
        with torch.no_grad():
    ...
    def ppo_step(self, query_ids, response_ids, rewards):
        """Esegue un passo di PPO."""
        # Converte rewards in tensore
        rewards = torch.tensor(rewards, dtype=torch.float32).to(self.device)
        
        # NON normalizzare i rewards - teniamoli come sono
        # Forward pass
        outputs = self.model(
            input_ids=query_ids,
        ...
        # Stima dei valori dalle hidden states del decoder
        values = self.value_head(decoder_hidden_states[:, -1, :])  # Shape: [batch_size]
        ...
        # Assicurati che il modello sia in training mode
        self.model.train()
...
for epoch in range(1):
    ...
    for batch_idx, sample in enumerate(tqdm(dataset)):
        ...
        if isinstance(response_ids, torch.Tensor):
            ...
            response_tensor = torch.tensor(response_ids).to(device)
